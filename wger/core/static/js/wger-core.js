    ...
    Sortable.create(element, {
      ...
      onUpdate: function(event) {
        ...
        dayId = $(event.target)
          .parents("table")
          .data("id");
        $.each($(event.from).children("tr"), function(
          eventIndex,
          eventElement
        ) {
          var trElement;
        ...
        $("#div-day-" + dayId).load(
          "/" + getCurrentLanguage() + "/workout/day/" + dayId + "/view/"
        );
      }
    });
  });
...
function wgerFormModalDialog() {
  ...
  $wgerModalDialog.click(function(e) {
    ...
    $ajaxInfoContent.load(targetUrl + " .form-horizontal", function(
      responseText,
      textStatus,
      XMLHttpRequest
    ) {
...
function wgerInitEditSet() {
  ...
  $("#id_categories_list").on("change", function() {
    ...
    $.get("/api/v2/language/?short_name=" + getCurrentLanguage(), function(
      data
    ) {
      var filter;
      var baseUrl;
      ...
      $.get(baseUrl + filter, function(exerciseData) {
        ...
        $idExerciseList = $("#id_exercise_list");
        $idExerciseList.find("option").remove();
