...
class ChoiceWithOtherRenderer(forms.RadioSelect.renderer):
    ...
    def __iter__(self):
        for input in super(ChoiceWithOtherRenderer, self).__iter__():
            yield input
        id = '%s_%s' % (self.attrs['id'], self.other[
                        0]) if 'id' in self.attrs else ''
        label_for = ' for="%s"' % id if id else ''
        checked = '' if not force_text(
            self.other[0]) == self.value else 'checked="true" '
        yield '<label%s><input type="radio"\
        id="%s"value="%s" name="%s" %s/> %s</label> %%s' % (
...
class ChoiceWithOtherField(forms.MultiValueField):
    ...
    The last item in the choices array passed to __init__ is expected to be a choice for "other".
    This field's cleaned data is a tuple consisting of the choice the user made,
    and the "other" field typed in if the choice made was the last one.
    """

    def __init__(self, *args, **kwargs):
        fields = [
            forms.ChoiceField(widget=forms.RadioSelect(
                renderer=ChoiceWithOtherRenderer), *args, **kwargs),
    ...
    def compress(self, value):
        ...
        if not value:
            return [None, u'']
        # Patch to override model specific other choice and return CharField
        # value instead of choice tuple
        if value[0] == 'Other':
            return value[1]
        else:
            return value[0]
